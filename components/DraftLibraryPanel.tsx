...
// If your generation endpoint differs, change this:
const GENERATE_API = "/api/generate";

function DashboardInner() {
  const sp = useSearchParams();

  // Seed from query (e.g. coming from /templates/[slug] LaunchFromTemplate)
  const initialPrompt = sp.get("prompt") ?? "";
  const initialSector = (sp.get("sector") ?? "") as SectorKey | "";
  const templateSlug = sp.get("template") ?? ""; // optional
  ...
  // If user lands with a prefilled prompt, we can auto-generate once (optional).
  useEffect(() => {
      ...
      <header className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-white/70">
            Generate documents, then save them to your Library.
          </p>
        </div>
      </header>
